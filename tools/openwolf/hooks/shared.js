...
export function countSemanticEntries(wolfDir, sessionStarted) {
    // A semantic entry is a non-mechanical table row. Two row shapes exist:
    // "| YYYY-MM-DD HH:MM | ..." and the "| HH:MM | ..." shape OPENWOLF.md
    // itself tells the agent to write. The old check only accepted the first,
    // so an entry written exactly as instructed was never counted. Rows count
    // when dated today, or when they sit under a "## Session:" header written
    // at or after this session started.
    const memoryPath = path.join(wolfDir, "memory.md");
    try {
        const content = fs.readFileSync(memoryPath, "utf-8");
        const mechanical = /^\|\s*(\d{4}-\d{2}-\d{2}\s+)?[\d:]+\s*\|\s*(Created|Edited|Multi-edited|Session end:|designqc:)/;
        const timeRow = /^\|\s*(\d{4}-\d{2}-\d{2}\s+)?\d{1,2}:\d{2}\s*\|/;
        const today = new Date().toISOString().slice(0, 10);
        const todayPrefix = `| ${today}`;
        const startedMs = sessionStarted ? Date.parse(sessionStarted) : NaN;
        const startedMinute = Number.isNaN(startedMs) ? NaN : Math.floor(startedMs / 60000) * 60000;
        let inSession = false;
        let count = 0;
        for (const line of content.split("\n")) {
            const header = line.match(/^## Session: (\d{4}-\d{2}-\d{2}) (\d{1,2}):(\d{2})/);
            if (header) {
                // Header times are local (timeShort); compare in local time.
                const [, date, hh, mm] = header;
                const [y, mo, d] = date.split("-").map(Number);
                const headerMs = new Date(y, mo - 1, d, Number(hh), Number(mm)).getTime();
                inSession = !Number.isNaN(startedMinute) && headerMs >= startedMinute - 60000;
                continue;
            }
            if (mechanical.test(line) || !timeRow.test(line))
                continue;
            if (line.startsWith(todayPrefix) || inSession)
                count++;
        }
        return count;
    }
