...
from scraper_engine.config.schema import DlqReaperConfig
...
class TestIsEligible:
    ...
    async def test_proxy_exhausted_eligible_when_tier_healthy(self, monkeypatch):
        ...
        assert await dlq_reaper._is_eligible(entry, redis, cb) is True
    ...
    async def test_proxy_exhausted_not_eligible_when_tier_still_degraded(self, monkeypatch):
        ...
        assert await dlq_reaper._is_eligible(entry, redis, cb) is False
...
class TestRetryEntry:
    ...
    async def test_skips_reenqueue_when_job_already_active(self, tenant):
        """A job that's already PENDING/PROCESSING for some other reason
        must not get a duplicate rq job stacked on top of it."""
        pg = AsyncMock()
        pg.fetchrow.return_value = None  # UPDATE matched 0 rows (not FAILED/DEAD_LETTER)
        dlq = AsyncMock()
        queue = MagicMock()  # rq.Queue.enqueue is a sync call
...
class TestReapTenant:
    ...
    async def test_only_retries_eligible_candidates(self, tenant, monkeypatch):
        ...
        async def fake_is_eligible(entry, redis_arg, cb_arg):
        ...
        pg = AsyncMock()
        count = await dlq_reaper._reap_tenant(pg, redis, cb, queue, tenant, cfg)
...
class TestReapCycle:
    ...
    async def test_sums_across_tenants_and_isolates_failures(self, monkeypatch):
        ...
        async def fake_reap_tenant(pg_arg, redis_arg, cb_arg, queue_arg, tenant_arg, reaper_cfg):
