...
Project identity, operating rules, and navigation. Currently at round 38 (grew free proxy-harvest source breadth from 8 to 12 sources, fixed a starvation bug in `_direct_scrape`'s early-break-on-limit, then — pushed by the user to verify rather than accept the long-standing "free sources can't reach L3" assumption — found and fixed the two real scoring bugs actually behind it: a judge-validation latency measurement polluted by dead-candidate timeout time, and `response_time_ms` frozen at its first-ever reading instead of refreshed by later health checks; live result: pool's first-ever L3-caliber proxies, 0→5, and L2-caliber single-digits→43, after one health cycle). Round 37: six-layer L2/L3 proxy-leasing reliability fix — TCP+HTTPS-CONNECT preflight, SQL-side candidate exclusion, same-level retry with a fresh proxy, Camoufox geoip-launch fallback, and a fix to round 34's DLQ auto-retry eligibility that had been silently disabled for this repo's exact deployment shape — closing a 150s+-hang regression round 35's scoring fix exposed, each further layer found via live re-verification of the previous one. Full round-by-round narrative, every open thread, every decision: `.claude/knowledge/technical-debt.md` (start there, not here — this file is navigation only). Source code is fully implemented — not blueprint phase.
