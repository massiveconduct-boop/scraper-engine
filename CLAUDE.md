...
|---|---|
| `core/` | Domain models, TenantId, SSRF guard, retry, budget, quota. `periodic.py` — shared polling-loop helper (with optional Redis liveness heartbeat) reused by the webhook sweeper and DLQ reaper. `budget.py` — `XVFB_LOCK` (process-wide lock serializing headful-browser display spinup/teardown) and `resolve_browser_max_total_instances()` (opt-in RAM-aware ceiling on live browser instances). `models.py` — `Proxy` (paid-gateway auth fields), `FetchResult` (`network_events` from Botasaurus CDP capture). `startup.py` — `wait_for_dependency()`, the unbounded-by-default dependency wait every process's startup uses instead of exiting when Postgres/Redis/S3 isn't up yet |
...
| `orchestrator/` | `Worker` — escalation state machine: cache-reuse check, cooperative cancellation, DLQ transient/permanent split, two independent retry budgets (one same-level free-pool retry on proxy-attributable failure; a separate `rotate_on_block_retries` gateway budget that rotates the exit IP on a detected block), concurrent URL dispatch bounded by `politeness.max_concurrent_urls_per_job` (default 5, `asyncio.Semaphore`). `CircuitBreaker`, `PolitenessController`, `job_queue.py` (rq producer), `tasks.py` (rq consumer — persists each result as it lands, webhook outbox, `network_events` column), `stuck_job_reaper.py` (reconciles PENDING/PROCESSING rows against rq — checks real queue/registry reachability, not just the job hash's status field), `WebhookDispatcher`, `webhook_events.py`/`webhook_dispatch.py`/`webhook_sweeper.py`/`slack_formatter.py` |
...
| `cli/` | Entrypoint. Ops subcommands (`serve`/`worker`/`harvest`/`reap`/`check`/`create-tenant`) talk directly to Postgres/Redis. Caller-facing `api` subcommand group (`scrape`/`jobs`/`job`/`quota`/`dlq`) wraps the real `/v1` HTTP API via `httpx` instead of touching storage directly. Outside `pyproject.toml`'s coverage-gated source, has its own test file |
...
| `scrapy_project/` | Scrapy spider project backing `services/scrapy_adapter.py`'s subprocess-isolated bulk-crawl path |
