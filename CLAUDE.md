...
Project identity, operating rules, and navigation. Currently at round 60.
...
- **Evolution history (round-by-round narrative moved out of this file in
  ...
  launch (round 57); fixed the one real adjacent bug that recon had found
  but not implemented — Botasaurus's fetch path never autoscrolled, so
  lazy-load/infinite-scroll content was silently dropped even with
  `scroll_passes>0` configured — live-verified with a real browser launch
  showing 0→100 lazy-loaded items after scroll; the round's other 6
  recon'd feature gaps stayed out of scope per explicit instruction not to
  bundle unrelated work into a bug fix (round 58); picked 2 of those
  remaining 6 backlog items on engineering judgment — `block_images`/
  `block_images_and_css` Driver kwargs, and an opt-in RAM-aware
  `BROWSER_SEMAPHORE` concurrency cap calibrated against a real measured
  Botasaurus/Chromium RSS (~0.79GB) — both live-verified for real, the
  other 4 (human mouse simulation, `extensions`/`lang` kwargs, raw CDP
  events) stay open backlog (round 59); closed all 4 remaining items —
  `extensions`, `lang`/locale/timezone spoof, human-mode mouse simulation,
  raw CDP network events (new `network_events` migration on
  `scrape_results`) — each as an independent unit per explicit
  instruction not to bundle them, all live-verified against a real
  browser; live testing found `Driver(lang=...)` does not actually spoof
  `navigator.language` on the installed Chromium build despite its own
  docstring's claim (kept, documented honestly — see decisions.md), and
  found the real `FetchResult`-construction site for level 2 is
  `fetcher/level_2.py`, not `orchestrator/worker.py` as originally planned
  (round 60). Current design, topic-
...
|---|---|
| `core/` | Domain models, TenantId, SSRF guard, retry, budget, quota, `periodic.py` (round 34 — shared `run_periodic` loop helper, extracted from `proxy/harvester_daemon.py` so `orchestrator/webhook_sweeper.py` and `proxy/dlq_reaper.py` reuse it instead of re-implementing; round 36 — optionally writes a Redis liveness heartbeat after every cycle attempt when a `redis` client is passed; round 40 — `models.py::Proxy` gained auth fields for the paid-gateway proxy, see architecture.md → "Paid Gateway Proxy"), `budget.py` (round 41 — new `XVFB_LOCK`, a process-wide `asyncio.Lock` serializing every headfull-browser Xvfb spinup/teardown across both Botasaurus and Camoufox, closing round 40's open display-contention crash — see architecture.md → "Xvfb Display-Contention Lock"; round 59 — new `resolve_browser_max_total_instances()`, opt-in RAM-aware `BROWSER_SEMAPHORE` ceiling delegating to botasaurus's own `calc_max_parallel_browsers()`, calibrated against a real measured Botasaurus/Chromium headful RSS (~0.79GB, heavier than Camoufox's own 80MB figure above it since the semaphore is shared) — can only reduce the configured ceiling, never raise it, see technical-debt.md round-59 entry), `models.py::FetchResult` (round 60 — new `network_events: list[dict] | None`, populated by `fetcher/level_2.py` when `config.botasaurus.capture_network_events` is on, persisted via a new migration — see `browser/`/`orchestrator/` rows below) |
