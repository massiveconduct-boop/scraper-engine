...
| `orchestrator/` | `Worker` — escalation state machine: cache-reuse check, cooperative cancellation, DLQ transient/permanent split, one same-level proxy retry on proxy-attributable failure, concurrent URL dispatch bounded by `politeness.max_concurrent_urls_per_job` (default 5, `asyncio.Semaphore`). `CircuitBreaker`, `PolitenessController`, `job_queue.py` (rq producer), `tasks.py` (rq consumer — persists each result as it lands, webhook outbox, `network_events` column), `WebhookDispatcher`, `webhook_events.py`/`webhook_dispatch.py`/`webhook_sweeper.py`/`slack_formatter.py` |
