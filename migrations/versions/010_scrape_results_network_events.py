# migrations/versions/010_scrape_results_network_events.py
"""Add network_events to scrape_results.

Round 60 — opt-in raw CDP network-event capture (config.botasaurus.
capture_network_events), via driver.before_request_sent()/
after_response_received() hooks in browser/botasaurus_pool.py and
fetcher/botasaurus_wrapper.py. FetchResult.network_events needed a real
persisted column — json_data is already committed to
result.extracted (extraction-schema output), not a free-form bucket, so
this is a new additive column, following the same create_tenant_schema
function-based pattern as 004-009.

Updates create_tenant_schema() so new tenants get the column from the
start, and backfills every existing tenant schema in place.
"""
...
def upgrade() -> None:
    # ── Redefine create_tenant_schema() with network_events added to
    # scrape_results — identical to 009's current definition otherwise. ──
    op.execute("""
            ...
            LOOP
                EXECUTE format(
                    'ALTER TABLE IF EXISTS %I.scrape_results
                        ADD COLUMN IF NOT EXISTS network_events JSONB',
                    tenant_schema
                );
            END LOOP;
        END;
...
def downgrade() -> None:
    op.execute("""
        ...
        BEGIN
            ...
            LOOP
                EXECUTE format(
                    'ALTER TABLE IF EXISTS %I.scrape_results DROP COLUMN IF EXISTS network_events',
                    tenant_schema
                );
            END LOOP;
        END;
