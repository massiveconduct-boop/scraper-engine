# Round 8 Closure — Per-Tenant Quota Enforcement
...
Raises `QuotaExceededError`, never returns bool. `__init__` accepts `redis` + optional `daily_limit` with `DEFAULT_DAILY_LIMIT = 10_000` fallback. The `tenants.quota_daily_limit` column existed but was never read — dead column. Now wired.

## 2. Code Fix — `api/routes.py` (complete, per-tenant quota)
...
async def scrape(
    ...
    if _storage_redis is not None and _storage_pg is not None:
        ...
        # Read per-tenant limit; defaults to 10_000 if tenant has no row.
        # tenants is a global table in public schema, not per-tenant.
        daily_limit = None
...
async def get_job(
    ...
    if _storage_pg is None:
        return JobStatusResponse(
            job_id=job_id, status=JobStatus.PENDING, progress=0.0,
        )
...
**Fixes applied:**
- `daily_limit=3` hardcode removed
- Per-tenant limit read from `public.tenants.quota_daily_limit`
- Falls back to `QuotaManager.DEFAULT_DAILY_LIMIT = 10_000` when no row exists for the tenant
- Catches only `QuotaExceededError` → 429; no bare except

## 3. Three-Curl Evidence — Per-Tenant Limits
...
### system (limit=2) — 200, 200, 429 at 17:52:02Z
...
Two tenants, two distinct limits, independent enforcement. Per-tenant isolation confirmed.
...
Restored: quota_daily_limit = 10000 for system and other
```
